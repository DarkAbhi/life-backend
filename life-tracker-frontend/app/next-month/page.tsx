...
export default function NextMonth() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const response = await fetch(`${apiBaseURL}/api/next-month-purchases`, { credentials: "include" });
        if (!response.ok) {
          setError("We couldn't load your planned purchases. Please try again.");
          return;
        }
  ...
  async function deletePurchase(item: Item) {
    setError("");
    setDeletingID(item.id);
    try {
      const response = await fetch(`${apiBaseURL}/api/next-month-purchases/${item.id}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (!response.ok) {
        setError("We couldn't delete that purchase. Please try again.");
        return;
      }
      setItems((current) => current.filter((purchase) => purchase.id !== item.id));
  ...
  async function clearAll() {
    ...
    try {
      ...
      if (!response.ok) {
        setError("We couldn't clear your planned purchases. Please try again.");
        return;
      }
      setItems([]);
      setTotal(0);
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <header className="mt-6 flex items-start justify-between gap-4">
          ...
          <button
            ...
            disabled={loading || isClearing || deletingID !== null || items.length === 0}
            onClick={() => {
              setError("");
              ...
              <article className="flex items-center justify-between gap-4 rounded-2xl bg-white p-5 shadow-sm" key={item.id}>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button
                    ...
                    disabled={isClearing || deletingID !== null}
                    onClick={() => {
                      setError("");
        ...
        <div className="fixed inset-0 z-10 flex items-center justify-center bg-stone-950/40 px-6" role="dialog" aria-labelledby="delete-purchase-title" aria-modal="true">
          <section className="w-full max-w-md rounded-2xl bg-white p-8 shadow-2xl">
            ...
            <div className="mt-6 flex gap-3">
              ...
              <button className="flex-1 rounded-lg bg-red-700 px-4 py-3 text-sm font-semibold text-white transition hover:bg-red-800 disabled:cursor-not-allowed disabled:bg-red-300" disabled={deletingID !== null} onClick={() => void deletePurchase(purchaseToDelete)} type="button">{deletingID !== null ? "Deleting…" : "Delete purchase"}</button>
            </div>
          </section>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-10 flex items-center justify-center bg-stone-950/40 px-6" role="dialog" aria-labelledby="clear-purchases-title" aria-modal="true">
          <section className="w-full max-w-md rounded-2xl bg-white p-8 shadow-2xl">
            ...
            <div className="mt-6 flex gap-3">
              ...
              <button className="flex-1 rounded-lg bg-red-700 px-4 py-3 text-sm font-semibold text-white transition hover:bg-red-800 disabled:cursor-not-allowed disabled:bg-red-300" disabled={isClearing} onClick={() => void clearAll()} type="button">{isClearing ? "Clearing…" : "Clear all"}</button>
            </div>
          </section>
        </div>
      )}
