...
export default function GymVisit() {
  ...
  useEffect(() => {
    async function loadVisit() {
      try {
        ...
        const response = await fetch(`${apiBaseURL}/api/gym-visits/${visitID}/exercises`, {
          credentials: "include",
        });
  ...
  function updateSet(index: number, field: keyof ExerciseSet, value: string) {
    setSets((currentSets) =>
      currentSets.map((set, setIndex) => (setIndex === index ? { ...set, [field]: value } : set)),
    );
  }
  ...
  async function saveExercise(event: SubmitEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const body = (await response.json()) as SavedExercise & { error?: string };
      if (!response.ok) {
    ...
    <main className="min-h-screen bg-[#fffaf3] px-6 py-10 text-stone-800 sm:px-10 lg:px-16">
      <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[1.15fr_0.85fr]">
        <section>
          ...
          <p className="mt-5 text-sm font-semibold tracking-[0.18em] text-emerald-700">GYM VISIT</p>
          <h1 className="mt-3 text-3xl font-bold tracking-tight text-stone-900 sm:text-4xl">Today&apos;s workout</h1>
          <p className="mt-3 text-base text-stone-600">Capture what you did, one exercise and set at a time.</p>
          ...
          <div className="mt-8 space-y-4">
            {isLoading ? (
              ...
              <div className="rounded-2xl border border-dashed border-emerald-200 bg-white/70 p-8">
                <p className="font-semibold text-stone-800">No exercises saved yet.</p>
                <p className="mt-2 text-sm text-stone-600">Add your first exercise using the form.</p>
              </div>
            ) : (
              exercises.map((exercise) => (
                <article className="rounded-2xl border border-emerald-100 bg-white p-6 shadow-sm" key={exercise.id}>
                  <h2 className="text-xl font-semibold text-stone-900">{exercise.name}</h2>
                    ...
                    <table className="w-full text-left text-sm">
                      ...
                      <tbody>
                        {exercise.sets.map((set) => (
                          <tr className="border-t border-stone-100" key={set.id}>
                            <td className="px-4 py-3">{set.set_number}</td>
                            <td className="px-4 py-3">{set.reps}</td>
                            <td className="px-4 py-3">{set.weight === null ? "Bodyweight" : `${set.weight} kg`}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
        ...
        <aside className="h-fit rounded-2xl border border-emerald-100 bg-white p-6 shadow-sm sm:p-8">
          <h2 className="text-xl font-semibold text-stone-900">Add an exercise</h2>
          <form className="mt-6 space-y-5" onSubmit={saveExercise}>
            <div>
              <label className="mb-2 block text-sm font-medium text-stone-700" htmlFor="exercise-name">Workout name</label>
              <input
            ...
            <fieldset className="space-y-3">
              <legend className="text-sm font-medium text-stone-700">Sets</legend>
              {sets.map((set, index) => (
                <div className="grid grid-cols-[auto_1fr_1fr] items-end gap-2" key={index}>
                  <span className="pb-3 text-sm font-semibold text-stone-500">{index + 1}</span>
