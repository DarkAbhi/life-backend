...
func (a *API) AddWorkoutForDay(w http.ResponseWriter, r *http.Request) {
	...
	switch {
	case err == nil:
		badRequest(w, "Gym entry already marked for today.")
		return
	case errors.Is(err, sql.ErrNoRows):
		_, err := a.DB.Exec(`INSERT INTO gym_visits DEFAULT VALUES;`)
		if err != nil {
			serverError(w, err)
			return
		}
...
func (a *API) UpdateVehicle(w http.ResponseWriter, r *http.Request) {
    ...
    if p.Name != nil { curName = *p.Name }
    if p.IsActive != nil { curActive = *p.IsActive }
